document.write("<link rel=\'stylesheet\' href=\'src/alert.css\' type=\'text/css\'/>");
var my = new function() {
	this.width = 300;
	this.height = 172;
	...
	function messageBox(html, title, message, type) {
		var jq = $(html);
		if(type == "toast") {
			jq.find(".window-panel").width(message.length * 20).css("margin-left", -message.length * 20 / 2).css("margin-top", -my.height / 2);
		} else {
			jq.find(".window-panel").width(my.width).css("margin-left", -my.width / 2).css("margin-top", -my.height / 2 - 36);
		}
		if(valempty(title)) {
