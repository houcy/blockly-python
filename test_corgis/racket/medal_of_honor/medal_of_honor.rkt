...
(define-struct awarded
    (issued citation general-order-number accredited-to location date))
...
(define (json->birth jdata)
    (make-birth
        (json->date (hash-ref jdata (string->symbol "date")))
                (hash-ref jdata (string->symbol "location name"))
            ))
...
(define (json->awarded jdata)
    (make-awarded
        (hash-ref jdata (string->symbol "issued"))
            ...
            (hash-ref jdata (string->symbol "General Order number"))
