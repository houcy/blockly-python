...
(define-struct data
    (niacin major-minerals vitamins selenium thiamin choline beta-carotene fiber refuse-percentage fat water lycopene pantothenic-acid ash household-weights retinol manganese beta-cryptoxanthin riboflavin cholesterol lutein-and-zeaxanthin carbohydrate kilocalories alpha-carotene sugar-total protein))
...
(define-struct major-minerals
    (copper zinc sodium potassium calcium magnesium iron phosphorus))

(define-struct household-weights
    (2nd-household-weight 2nd-household-weight-description 1st-household-weight 1st-household-weight-description))
...
(define (json->data jdata)
    (make-data
        (hash-ref jdata (string->symbol "Niacin"))
            ...
            (hash-ref jdata (string->symbol "Carbohydrate"))
