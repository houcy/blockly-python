...
(define database-name "county_demographics.db")
...
(define-struct report
    (ethnicities age miscellaneous sales county state income education employment housing population))
...
(define (get-all-counties [test #t]) 

    (if (or test-mode test)
        (let* [(result (query-list database 
                                  (format "SELECT data FROM county_demographics LIMIT ~a" hardware-limit)
                                ))
               (result (map string->jsexpr result))]
           (map json->report result)
           )(let* [(result (query-list database 
                                   "SELECT data FROM county_demographics"
                                ))
               (result (map string->jsexpr result))
               ]
           (map json->report result)
...
(define (main)
    ...
    (displayln "Production get-all-counties")
    (displayln (length (time (get-all-counties false))))
    (displayln "Test get-all-counties")
    (displayln (length (time (get-all-counties true))))
